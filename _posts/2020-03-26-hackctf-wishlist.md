...
1.  **시나리오의 1~3 ( heap_base 주소를 아는것이 목적임 )**
    ...
    uaf를 이용해서 ptr[3]에 현재 ptr[1]에서 할당받았던 청크주소가 들어가 있다. fd가 그대로 있기 때문에 view를 호출하여 index 3번을 선택하면
...
6. **스택 피보팅으로 fake stack 조지기 !**
    ...
    - pop이 진행됬으므로 rsp가 증가하여 0x...c78이 된다
    ...
    - 결국 여기서 ret이 실행되면 다시 leave로 갈것이다.
    ...
    - 그리고 pop rbp가 진행되면서 rsp가 가리키는 값인 0x21이 rbp에 들어가고 rsp는 8증가해 0x12efcf0이 될 것이다
    ...
    - ret가 진행되면 현재 rsp가 가리키는 값인 0x400b03가 rip들어갈 것이고, 이는 pop rdi 주소이다
